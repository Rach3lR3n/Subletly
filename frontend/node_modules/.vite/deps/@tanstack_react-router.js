...
function useMatchRoute() {
	const router = useRouter();
	if (!router.isServer) useStore(router.stores.matchRouteReactivity, (d) => d);
...
function useCanGoBack() {
	const router = useRouter();
	if (router.isServer) return router.stores.location.state.state.__TSR_index !== 0;
...
var useTags = (assetCrossOrigin) => {
	...
	if (router.isServer) return buildTagsFromMatches(router, nonce, router.stores.activeMatchesSnapshot.state, assetCrossOrigin);
